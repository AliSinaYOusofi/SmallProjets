...
const weatherIcons = ["icons8-cloud-100.png", "icons8-clouds-and-sun-100.png", "icons8-cloudy-day-100.png",
                      "icons8-cloudy-day-1001.png", "icons8-haze-96.png", "icons8-rainy-day-100.png",
                       "icons8-snow-96.png", "icons8-sun-star-96.png",
...
searchButton.addEventListener("click", tar => {
   ...
   fetch("https://api.openweathermap.org/data/2.5/weather?q=" + city.value +"&appid=" + api_key).then(
       response => ( response.json())
        ...
        data => {
            ...
            if (data['weather'][0].main === "Clouds")
                ...
                currentWeather.setAttribute("src", "icons8-umbrella-with-rain-drops-96.png");
...
function convertToFare(degree) { return ((degree - 273.15) * 9/5) + 32;}
function randomIconSelector(min, max) { return Math.ceil(Math.random() * (max - min + 1) + min);}
function setRandomIcons() {
    sunny.setAttribute("src", weatherIcons[randomIconSelector(0, weatherIcons.length - 1)]);
    cloudy.setAttribute("src", weatherIcons[randomIconSelector(0, weatherIcons.length - 1)]);
    rainy.setAttribute("src", weatherIcons[randomIconSelector(0, weatherIcons.length - 1)]);
    snow.setAttribute("src", weatherIcons[randomIconSelector(0, weatherIcons.length - 1)]);
}